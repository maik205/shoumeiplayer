...
class MpvTrackListTest {
    ...
    @Test
    fun `parse keeps video audio and subtitle tracks`() {
        val tracks = MpvTrackList.parse(trackListJson)

        // Video tracks are exposed for the TV quality/details panel alongside audio and subtitles.
        assertEquals(5, tracks.size)
        assertEquals(0, tracks.single { it.type == TrackType.VIDEO }.ffIndex)
