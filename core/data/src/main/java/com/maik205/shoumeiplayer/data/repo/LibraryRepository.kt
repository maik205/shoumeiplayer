...
import com.maik205.shoumeiplayer.domain.result.flatMap
...
class LibraryRepository(private val client: JellyfinClient) {
    ...
    suspend fun resolvePlayableTarget(item: BaseItemDto): ApiResult<PlayableTarget> {
        ...
        return resolveSeriesPlayableTarget(item.id)
    }

    suspend fun resolveSeriesPlayableTarget(seriesId: String): ApiResult<PlayableTarget> {
        val nextEpisode = (nextUp(limit = 1, seriesId = seriesId) as? ApiResult.Success)
            ?.data
            ?.firstOrNull()
        if (nextEpisode != null) {
            return ApiResult.Success(
        ...
        return episodes(seriesId, seasonId = null).flatMap { episodeList ->
            val target = episodeList.firstOrNull { episode ->
                ...
                ?: return@flatMap ApiResult.Failure(
                    ApiError.Http(404, "Series $seriesId contains no playable episodes"),
                )
            ApiResult.Success(
