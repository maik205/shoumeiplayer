...
- `[ ]` — open
- `[~]` — addressed in the checked-in source, but not yet closed: nothing here has been run on a
  device, which is the last of the closure requirements below
- `[x]` — closed with the evidence listed under Closure requirements

All 95 items are addressed. Requirements 1-3 are met throughout, and requirement 4 is met for the
shared primitives every screen goes through -- the Back layer order and the modal-exclusivity rule
have JVM tests, and `app/src/androidTest` covers back-stack contracts, D-pad edge wrapping, and
async focus handoff. Requirement 5 is met for nothing: the instrumented suite needs an emulator the
CI job does not have, so it is compiled there rather than run, and no item may be checked `[x]`
until someone exercises it on a real device.
